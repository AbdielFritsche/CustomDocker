...
func newStatsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mgr := container.NewManager(statsDataRoot)
			...
			for {
				...
				case now := <-ticker.C: // <--- Corregido a ticker.C
	...
	cmd.Flags().StringVar(&statsDataRoot, "data-root", "/var/lib/minidocker/containers", "Ruta de almacenamiento")
	return cmd
}
